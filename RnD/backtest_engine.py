"""
Backtest Engine - Analyzes historical signal outcomes.

This engine takes historical signals and analyzes what happened after each signal,
measuring price movements across multiple time horizons.
"""
...
from .historical_signal_simulator import HistoricalSignal
from .experiment_result import ExperimentResult, SignalOutcome
...
class BacktestConfig:
    ...
    # Time horizons to analyze (in candles)
    horizons: List[int] = field(default_factory=lambda: [1, 3, 5, 10, 20])
    
    # Success/failure thresholds (percentage)
    success_threshold: float = 1.0  # 1% move in favorable direction
    failure_threshold: float = 1.0  # 1% move in adverse direction
    
    # Neutral zone (if move is within this %, it's neutral)
    neutral_zone: float = 0.5
    
    # Maximum candles to look ahead for outcome
...
class BacktestEngine:
    """
    Analyzes historical signals to determine outcomes.
